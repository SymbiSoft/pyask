...
def zwrocliste():
    k=[]
    for i in slowa():
        k.append(unicode(str(i[0])+'-'+str(i[1])))
    return k
    ...
    if zwrocliste() == []:
        ...
        if index2 == ():
            ...
            if czynapewno:
                ...
                f=open(path, 'a')
                for i in noweslowa:
                    f.write(str(i[0])[2:-2] + '-' + str(i[1])[2:-2] + '\r\n')
                f.close()
...
def menu():
    ...
    #     audio.say(unicode(k[0]))
            pytaj = appuifw.query(unicode('#' + str(wszystkie) + '/' + str(len(w)) + ' ' + str(k[strona])[1:-1]) + ' (' + str(len(k[strona2])) + ')',"text")
